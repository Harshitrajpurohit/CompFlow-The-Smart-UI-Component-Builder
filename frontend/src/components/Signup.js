...
export default function Signup({setUserExists}) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_SERVER_API}/api/signup`,{
        method:'POST',
        headers:{
    ...
    <div
      ...
      <form className="my-4" onSubmit={handleSubmit}>
        ...
        <LabelInputContainer className="mb-8">
          ...
          <Input id="Confirm" name="confirmPassword" placeholder="••••••••" type="Confirm" />
        </LabelInputContainer>
      ...
      <button onClick={() => setUserExists(true)} className="cursor-pointer text-center w-full underline"> Already have account? </button>
    </div>
  );
}
